...
class R3_Admin extends CI_controller{
	...
	public function courseinfo($cid = '0'){
		...
		$this->load->view('r3/r3_a_courseinfo',$data); //加载课程简介的视图，并且把结果传过去
	}
	//强行为某门课添加学生或删除学生
	...
	public function selectengine($tid = '0'){
		...
		$this->load->model('r3/r3_Course_model', 'coursem'); //加载course_model
		$this->load->model('r3/r3_Curriculum_model'); //加载curriculum_model
		...
		$this->load->view('r3/r3_a_selectengine', $data); //加载搜索引擎的视图，并且把结果传过去
	}
	...
	public function selectpage($cid = 0){
		...
		$this->load->view('r3/r3_a_selectpage',$data); //加载选课页面视图，并且把结果传过去
	}

	//管理员为某门教学班增删学生
	public function classmember($cid){
		$this->load->model('r3/r3_Curriculum_model','currim');
		$this->load->model('r3/r3_student_model','studem');
		$this->load->model('r3/r3_class_model','classm');
		//以下先拿到该教学班当前的学生列表
		...
		$this->load->view('r3/r3_a_classmember',$data); //加载增删学生的视图，并且把结果传过去
