...
class R3_Student extends CI_controller{ 
	...
	public function sprint($sid = 0){
		...
		*/
		...
		$this->load->model('r3_Curriculum_model','Curriculum_model');
	...
	public function teacherinfo($tid = ''){
		$this->load->model('r3_Class_model', 'classm');
		$this->load->model('r3_Teacher_model', 'teacherm');
		...
		$this->load->view('r3_s_teacherinfo', $data);
		
	}
	//学生评价课程
	public function assess($sid = 0){
		$this->load->model('r3_Score_model', 'scorem');
		$this->load->model('r3_Class_model', 'classm');
		...
		$this->load->view('r3_s_assess', $data);
	}
	...
	public function selectpage($sid = 0 ,$cid = 0){
		...
		*/
		...
		$this->load->model('r3_Class_model','classm');
		$this->load->model('r3_Curriculum_model','currim');
		...
		for ($i = 0; $i < 20; $i++)
			echo $hour;
		*/
		$this->load->view('r3_s_selectpage',$data);
	}
	...
	public function modifyclass($sid = 0 , $cid  = 0){
		...
		*/
		...
		$this->load->model('r3_Class_model','classm');
		$this->load->model('r3_Curriculum_model','currim');
		$this->load->model('r3_Selecttime_model', 'systime');
		...
		*/
		...
		$this->load->model('r3_Plan_model', 'planm');
		$this->load->model('r3_Course_model', 'coursem');
		...
		$this->load->model('r3_Student_model','studentm');
		...
		if ($goodtime == 0){
			...
			$data['sid'] = $sid;
			$data['cid'] = $cid;
			$this->load->view('r3_s_alert', $data);
		}
	...
	public function selectengine($sid = '0'){
		...
		//再把该学生已经选上的所有教学班查出来
		$res2 = $this->r3_Curriculum_model->get_all_course($sid);
		//var_dump($res1);
		$n = count($res1);
		$nj = count($res2);
		$data["coursenum"] = $n;
		...
		$this->load->view('r3_s_selectengine', $data); //加载搜索引擎的视图，并且把结果传过去
	}
	...
	*/
		...
		$this->load->view('r3_s_selectmajor',$data);
	}
	public function selectall($sid = 0){
		...
		*/
		...
		$this->load->view('r3_s_selectall',$data);
	}
	...
	public function courseinfo($cid = '0'){
		...
		*/
		...
		$this->load->view('r3_s_courseinfo',$data);
	}
...
*/
	...
	public function curriculum($sid = 0){
		...
		*/
		...
		$this->load->view('r3_s_curriculum',$data);
